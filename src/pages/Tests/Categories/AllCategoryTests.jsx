...
const AllCategoryTests = () => {
    ...
    const fetchAllTests = async () => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/tests/get_tests_in_category/${guid}`,
          requestOptions
        );
        const result = await response.json();
        setAllTests(result.payload);
        setLoading(false);
      } catch (error) {
  ...
  const handleUnlinkTest = async () => {
    ...
    selectedTests.forEach((value, index) => {
      formData.append(`test_guid`, value);
    });
    ...
    try {
      const res = await fetch(
        `${BASE_URL}/tests/unlink_category`,
        requestOptions
      );
...
export default AllCategoryTests;
